      ...
      if ((RegExp(
        ...
        if (fpassword.text == spassword.text) {
          if (status == true) {
            ...
            ApiWrapper.showToastMessage("Требуется принять правила НОВАпарка");
          }
        } else {
          ApiWrapper.showToastMessage("пароли не совпадают");
        }
      } else {
        ApiWrapper.showToastMessage('Пожалуйста, введите действительный адрес электронной почты');
      }
    } else {
      ApiWrapper.showToastMessage("Пожалуйста, заполните обязательное поле!");
    }
  }
  ...
  Future<void> verifyPhone(String number) async {
    await FirebaseAuth.instance.verifyPhoneNumber(
      ...
      verificationCompleted: (PhoneAuthCredential credential) {
        ApiWrapper.showToastMessage("Авторизация завершена!");
      },
      verificationFailed: (FirebaseAuthException e) {
        ApiWrapper.showToastMessage("Сбой регистрации");
      },
      codeSent: (String verificationId, int? resendToken) {
        ApiWrapper.showToastMessage("Одноразовый код отправлен!");
        // verID = verificationId;
        Get.to(() => Verification(verID: verificationId, number: number));
      },
      codeAutoRetrievalTimeout: (String verificationId) {
        ApiWrapper.showToastMessage("Время ожидания закончилось!");
      },
    );
  }
}
